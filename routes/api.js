...
router.get('/find', async (req, res, next) => {
    ...
    if (apikey != 'Cofs') return res.json(loghandler.invalidKey)
...
router.get('/tiktod/stalk', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'gabrielcoofs') return res.json(loghandler.invalidKey)
...
router.get('/textmaker/game', async (req, res, next) => {
	...
	if(apikeyInput != 'Cofs') return res.json(loghandler.invalidKey)
        if (!theme) return res.json(loghandler.nottheme)
        if (theme != 'pubg' && theme != 'naruto') return res.json(loghandler.notheme)
        if (!text) return res.json(loghandler.nottext)
                ...
                body: `text_1=${text}&text_2=${text2}&login=OK`,
                    ...
                    if (!e) {
                        ...
                        $(".thumbnail").find("img").each(function() {
                            ...
                            fetch(encodeURI(`https://api.imgbb.com/1/upload?expiration=600&key=d52c8697c949fc73fb59f8d6ced000dd&image=${result}&name=${randomTextNumber}`))
                                .then(response => response.json())
                                .then(data => {
                                    var urlnya = data.data.url,
                                        delete_url = data.data.delete_url;
...
router.get('/muslim/tahlil', async (req, res, next) => {
	...
	if(apikeyInput != 'Cofs') return res.json(loghandler.invalidKey)
       ...
       fetch(encodeURI(`https://raw.githubusercontent.com/gabrielcoofs/resultsql/main/dados/dataTahlil.json`))
        .then(response => response.json())
        .then(data => {
        var result = data;
             res.json({
